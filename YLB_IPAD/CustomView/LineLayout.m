...
-(void)prepareLayout
{
    ...
    self.cellItemWidth = (screenWidth - dockWidth) * 0.3;
    //初始化
    self.itemSize = CGSizeMake(self.cellItemWidth, ItemHeight);
    self.scrollDirection = UICollectionViewScrollDirectionHorizontal;
    self.minimumLineSpacing = 100;
    CGFloat inset = (self.collectionView.frame.size.width - self.cellItemWidth) / 2;
...
//    UICollectionViewLayoutAttributes
    ...
    for (UICollectionViewLayoutAttributes *attrs in array) {
        ...
        //差距越小，缩放比例越大
        //根据与屏幕最中间的距离计算缩放比例
       CGFloat scale = 1 + (1 - ABS(itemCenterx - centerX) / self.collectionView.frame.size.width ) * 0.8;//比例值很随意，适合就好
        
        //用这个，缩放不会改变frame大小，所以判断可见范围就无效，item即将离开可见范围的时候，突然消失不见
        attrs.transform3D = CATransform3DMakeScale(scale, scale, 1.0);
        
        if (ABS(itemCenterx - centerX) < 20) {
            attrs.zIndex = 10;
        }
    }
