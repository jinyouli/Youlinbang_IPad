...
-(void)lockViewPrepare{
    ...
    for (NSUInteger i=0; i<9; i++) {
        ...
        circle.isOpenTracks = self.isOpenTracks;
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    [self.subviews enumerateObjectsUsingBlock:^(PCCircle *circle, NSUInteger idx, BOOL *stop) {
        if (CGRectContainsPoint(circle.frame, point)) {
            ...
            [weakSelf.circleSet addObject:circle];
        }
    }];
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    [self.circleSet enumerateObjectsUsingBlock:^(PCCircle *circle, NSUInteger idx, BOOL *stop) {
        ...
        if (weakSelf.type != CircleViewTypeSetting) {
    ...
    if (length < CircleSetCountLeast) {
        ...
        if ([gestureOne length] < CircleSetCountLeast) { // 接收并存储第一个密码
            ...
            if (equal){
                ...
                [personalSpaceHttpDAO setUserConfigInfoWithUserName:[SYLoginInfoModel shareUserInfo].userInfoModel.username WithAlias:nil WithMotto:nil WithHeadUrl:nil WithEmail:nil WithGesturePwd:gesture Succeed:^{
                    
                    [SYLoginInfoModel shareUserInfo].personalSpaceModel.gesture_pwd = gesture;
